...
"""
GRID = [map(lambda x: int(x, 10), row.split()) for row in RAW_GRID.strip().split('\n')]
WIDTH = HEIGHT = 20
...
Result = namedtuple("Result", ["x", "y", "product"])
...
class Euler:
    ...
    def test(self):
        # Test grid parsing, 20x20 matrix of ints
        assert len(GRID) == 20
        for row in GRID:
            assert len(row) == 20
            for col in row:
                assert type(col) == int
        # Largest product of length 1 is the highest number in the grid
        series1 = largest_product_of_series(1)
        assert series1.x == 2
        assert series1.y == 1
        assert series1.product == 99

        # Largest product of length 2 is the highest 2 neighboring numbers in the grid
        series2 = largest_product_of_series(2)
        assert series2.x == 3
        assert series2.y == 0
        assert series2.product == 97 * 99
